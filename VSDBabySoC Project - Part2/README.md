# 🚀 VSDBabySoC: RISC-V Project (Pre & Post Simulation & Synthesis Guide)
...
### 1. GTk Wave Setting.jpg (Pre-Synthesis Waveform with Context Menu)

* **Signals Visible:** `clk`, `reset`, `D[9:0]`, and `OUT` (digital) under the `vsdbabysoc_tb.uut.dac` scope, and `OUT` (real) under the `vsdbabysoc_tb.uut.dac.pll` scope (though the scope is likely `dac` for both OUT signals).
    * **CLK:** Input CLK signal of the RVMYTH core. This signal comes from the PLL, originally.
    * **reset:** Input reset signal of the RVMYTH core. This signal comes from an external source, originally.
    * **OUT** (Digital): Output OUT signal of the VSDBabySoC module. This signal comes from the DAC (due to simulation restrictions it behaves like a digital signal which is incorrect), originally.
    * **D[9:0]:** The 10-bit output `[9:0]` OUT port of the RVMYTH core. This port comes from the RVMYTH register #17, originally.
    * **OUT** (Real Datatype): A real datatype wire which can simulate analog values. It is the output wire `real OUT` signal of the DAC module. This signal comes from the DAC, originally.
* **Represents:** The initial or pre-synthesis (RTL) functional simulation result, specifically showing the process of configuring the DAC's analog output.
* **Key Insight:** The selected signal is the `real OUT` (the analog output from the DAC), which is being configured by changing its Data Format to **Analog → Step**. This is the correct method in GTKWave to display signals of the `real` datatype as a continuous stepped analog waveform.
...
### Post Synth GTKWave.jpg (Post-Synthesis Waveform)

* **Signals Visible:** `clk`, `reset`, `EN` (enable), `D[9:0]`, and the `real OUT` signal (analog waveform), plus numerous internal gate-level **wire** signals (e.g., `wire 3674`, `clkP_CPU_rd_valid_a3`).
    * **CLK:** Input CLK signal of the RVMYTH core. This signal comes from the PLL, originally.
    * **reset:** Input reset signal of the RVMYTH core. This signal comes from an external source, originally.
    * **D[9:0]:** The 10-bit output `[9:0]` OUT port of the RVMYTH core. This port comes from the RVMYTH register #17, originally.
    * **OUT** (Real Datatype): A real datatype wire which can simulate analog values. It is the output wire `real OUT` signal of the DAC module. This signal comes from the DAC, originally.
* **Represents:** The timing-accurate simulation result using the **gate-level netlist** (`post_synth_sim.vcd`) generated by Yosys and mapped to standard cells.
* **Key Insight:** The waveform pattern is functionally identical to the pre-synthesis result. The appearance of many low-level `wire` signals confirms that the design has been synthesized into individual gates (standard cells). The simulation now includes the intrinsic delays of these gates, validating that the design not only works logically but also maintains its correct function when modeled with real-world **timing delays**.
---
